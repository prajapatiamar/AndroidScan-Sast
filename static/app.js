...
const TAG_META = {
  'memory-leak': { label: '\uD83D\uDEE0 MEMORY LEAK', className: 'tag-memory-leak' },
  'performance': { label: 'PERFORMANCE', className: 'tag-performance' },
  'runtime-error': { label: 'RUNTIME ERROR', className: 'tag-runtime-error' },
};
const TRACKED_TAGS = ['memory-leak', 'performance', 'runtime-error'];

function renderTagBadges(tags) {
  if (!tags || !tags.length) return '';
  return tags.map(t => TAG_META[t] ? `<span class="tag-badge ${TAG_META[t].className}">${TAG_META[t].label}</span>` : '').join('');
}
const PAGE_SIZE = 150;
...
const TAG_TILE_ICON_BG = { 'memory-leak': 'rgba(242,109,141,.15)', 'performance': 'rgba(91,156,246,.15)', 'runtime-error': 'rgba(242,153,74,.15)' };
const TAG_TILE_ICON_FG = { 'memory-leak': '#F26D8D', 'performance': '#5B9CF6', 'runtime-error': 'var(--high)' };
const TAG_TILE_LABELS = { 'memory-leak': '🛠 Memory Leak Risks', 'performance': 'Performance Issues', 'runtime-error': 'Runtime Error Risks' };
const TAG_TILE_ICONS = {
  'memory-leak': '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8"><path d="M14.7 6.3a1 1 0 000 1.4l1.6 1.6a1 1 0 001.4 0l3.77-3.77a6 6 0 01-7.94 7.94l-6.91 6.91a2.12 2.12 0 01-3-3l6.91-6.91a6 6 0 017.94-7.94l-3.76 3.76z"/></svg>',
  'performance': '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8"><circle cx="12" cy="13" r="8"/><path d="M12 9v4l3 2M9 2h6"/></svg>',
  'runtime-error': '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8"><path d="M12 9v4M12 17h.01M10.3 3.9L2.6 17a1.8 1.8 0 001.5 2.7h15.8a1.8 1.8 0 001.5-2.7L13.7 3.9a1.8 1.8 0 00-3.4 0z"/></svg>',
};

function renderTagTiles(r) {
  const counts = r.tag_counts || {};
  const prevCounts = (r.previous_scan && r.previous_scan.tag_counts) || null;
  const section = document.getElementById('tagTilesSection');
  const relevant = TRACKED_TAGS.filter(t => (counts[t] || 0) > 0);
  if (!relevant.length) { section.classList.add('hidden'); return; }
  section.classList.remove('hidden');
  const html = relevant.map(t => {
    const n = counts[t] || 0;
    const prev = prevCounts ? prevCounts[t] : null;
    return `
      <button class="tile ${activeTag === t ? 'active' : ''}" data-tag="${t}">
        <div class="tile-icon" style="background:${TAG_TILE_ICON_BG[t]};color:${TAG_TILE_ICON_FG[t]}">${TAG_TILE_ICONS[t]}</div>
        <div class="tile-body">
          <div class="tile-num-row"><span class="tile-num" style="color:${TAG_TILE_ICON_FG[t]}">${n}</span>${trendArrow(n, prev, true)}</div>
          <div class="tile-label">${TAG_TILE_LABELS[t]}</div>
        </div>
      </button>`;
  }).join('');
  document.getElementById('tagTiles').innerHTML = html;
  document.querySelectorAll('#tagTiles .tile').forEach(tile => {
    tile.addEventListener('click', () => {
      activeTag = tile.dataset.tag;
      activeCategory = 'all';
      goToPage('findings');
    });
  });
...
function renderFindingsPage() {
  ...
  renderTagFilters(currentResult);
...
function renderTagFilters(r) {
  const wrap = document.getElementById('tagFilters');
  if (!wrap) return;
  const counts = { all: r.findings.length };
  TRACKED_TAGS.forEach(t => counts[t] = (r.tag_counts || {})[t] || 0);
  const relevant = TRACKED_TAGS.filter(t => counts[t] > 0);
  if (!relevant.length) { wrap.innerHTML = ''; return; }
  wrap.innerHTML = `<button data-tag="all" class="${activeTag === 'all' ? 'active' : ''}">All types (${counts.all})</button>` +
    relevant.map(t => `<button data-tag="${t}" class="tag-filter-btn ${TAG_META[t].className} ${activeTag === t ? 'active' : ''}">${TAG_META[t].label} (${counts[t]})</button>`).join('');
